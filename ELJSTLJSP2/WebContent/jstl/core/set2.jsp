...
<%--    <%
   ...
   ${mName }<br>
   <c:set var="mGender" value="${requestScope.userInfo.userGender}"/>
   ${mGender }<br>
   <c:set var="mEmail" value="${requestScope.userInfo.userEmail}"/>
   ${mEmail }<br>
   
    <hr> <hr> <hr>
   
   
   <h2>java코드에서 전달된 리스트 정보 출력</h2>
   ${requestScope.memberList.userName }<br>
